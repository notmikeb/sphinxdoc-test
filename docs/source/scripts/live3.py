...
g_registernotification1 = getattr(dll1, "RegisterNotification")
g_registernotification2 = getattr(dll1, "RegisterNotification2")
g_clearcapture = getattr(dll1, "ClearCapture")
g_savecapture = getattr(dll1, "SaveCapture")
...
def py_notification_func( one ):
  print("noti {}".format( repr(one) ))
  

def py_notification2_func( one, id):
  print("noti2 {} {}".format( repr(one), id ))

CMP1FUNC = CFUNCTYPE(None,  POINTER(c_void_p))
CMP2FUNC = CFUNCTYPE(None,  POINTER(c_void_p), c_int)
notification_func = CMP1FUNC( py_notification_func )
notification2_func = CMP2FUNC( py_notification2_func )


def save_capture(path):
    """ save a cfa file ex. r"d:\temp\aaaa.cfa" """
    g_savecapture( create_string_buffer(path.encode('utf8')) )
	  
def closelive():
    """ close the liveimport connection"""
    g_releaseliveimport()

releaseliveimport = closelive